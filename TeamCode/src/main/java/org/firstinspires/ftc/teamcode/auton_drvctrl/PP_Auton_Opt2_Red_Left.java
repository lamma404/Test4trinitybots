...
/* == Autonomous option 2 == Red team - Left side
...
public class PP_Auton_Opt2_Red_Left extends LinearOpMode {
    // drivetrain
    ...
    public void runOpMode() {
        ...
        // for 1st cone to ground junction
        ...
        // 2. go to cone stack to grab a red cone
        Trajectory conestacktask1 = m_drivetrain.trajectoryBuilder(trajectory_groundJunctiontask3.end().plus(new Pose2d(0,0,Math.toRadians(-85))))
                .forward(40)
           //     .back(8)
                .build();
        Trajectory conestacktask2 = m_drivetrain.trajectoryBuilder(conestacktask1.end())
                .back(4)
                .build();
        Trajectory conestacktask3 = m_drivetrain.trajectoryBuilder(conestacktask2.end().plus(new Pose2d(0,0,Math.toRadians(90))))
                .forward(26)
                .build();

        //3. put cone on terminal junction :)
        Trajectory terminaltask1 = m_drivetrain.trajectoryBuilder(conestacktask3.end())
                .back(1.5)
                .build();
        Trajectory terminaltask2 = m_drivetrain.trajectoryBuilder(terminaltask1.end().plus(new Pose2d(0,0,Math.toRadians(90))))
                .forward(44)
                .build();

        // 4. park at the signaled area
        Trajectory trajectory_before_park = m_drivetrain.trajectoryBuilder(terminaltask2.end().plus(new Pose2d(0,0)))
                .back(18)
                .build();

//        Trajectory trajectory_park = m_drivetrain.trajectoryBuilder(trajectory_before_park.end().plus(new Pose2d(0,0,Math.toRadians(-90))))
//                .forward(10)
//                .build();
...
//                .build();
        ...
        // 1. put the preload cone onto the ground junction
        moveElevator(1.5);
        ...
        // 2. go to cone stack to grab a red cone
        m_drivetrain.turn(Math.toRadians(-85));
        sleep(100);
        m_drivetrain.followTrajectory(conestacktask1);
        sleep(100);
        m_drivetrain.followTrajectory(conestacktask2);
        sleep(50);
        m_drivetrain.turn(Math.toRadians(90));
        sleep(100);

        // now lift the claw to get ready to grab a cone from stack
        moveElevator(BotConfig.LOW_JUNCTION - 8);
        sleep(1000);

        m_drivetrain.followTrajectory(conestacktask3);
        sleep(200);

        // grab a cone and lift it up to low junction height
        m_clampServo.setPosition(BotConfig.SERVO_CLOSE_POS);
        sleep(500);
        moveElevator(BotConfig.LOW_JUNCTION);
        sleep(500);

        //3. put cone on terminal junction :)

        // back 1 inch
        m_drivetrain.followTrajectory(terminaltask1);
        sleep(100);
        //turn toward to the terminal
        m_drivetrain.turn(Math.toRadians(70));
        // lower the claw to 2 inch
        moveElevator(2);
        // go forward 44 inch to the terminal
        m_drivetrain.followTrajectory(terminaltask2);
        m_clampServo.setPosition(BotConfig.SERVO_OPEN_POS);
        sleep(50);

        moveElevator(0);
        m_drivetrain.followTrajectory(trajectory_before_park);

        // hold the cone for now
        sleep(5000);
