...
public class PP_DrvCtrl_Opt1 extends OpMode {
    ...
    public void loop()
    {
        ...
        if (gamepad2.b) {  // low junction
            moveElevator(BotConfig.LOW_JUNCTION + 1);
        }
        if (gamepad2.a){  // ground
            moveElevator(0);
        }
        if (gamepad2.dpad_up){ // lift the elevator 1 inch by 1 inch
...
//            moveElevator(m_elevator_pos+1);
        ...
        if (gamepad2.dpad_down) // drop the elevator 1 inch by 1 inch
        {
//            moveElevator(m_elevator_pos-1);
            int curr = m_elevatorMotor.getCurrentPosition();
            moveElevator(ElevatorConstants.encoderTicksToInches(curr)-1);
    ...
    private void calDriveTrainPower()
    {
        ...
        m_leftFrontPower  = 0.5 * (axial + lateral + yaw);
        m_rightFrontPower = 0.5 * (axial + lateral - yaw);
        m_leftBackPower = 0.5 * (axial - lateral + yaw);
        m_rightBackPower  = 0.5 * (axial - lateral - yaw);
